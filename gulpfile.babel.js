// Generated on 2020-10-05 using generator-angular-fullstack 4.2.2
'use strict';
...
gulp.task('env:all', function(done) {
    let localConfig;
    try {
...
gulp.task('inject', gulp.series(cb => {
...
gulp.task('webpack:dist', function() {
    ...
    return gulp.src(webpackDistConfig.entry.app)
        ...
        .pipe(gulp.dest(`${paths.dist}/server/client`));
});
...
gulp.task('start:client', gulp.series(cb => {
    whenServerReady(() => {
...
gulp.task('serve:debug', gulp.series(cb => {
    runSequence(
        [
            'clean:tmp',
            'lint:scripts',
...
gulp.task('serve:dist', gulp.series(cb => {
    runSequence(
        'build',
        'env:all',
...
gulp.task('test:server', gulp.series(cb => {
    runSequence(
        'env:all',
        'env:test',
        'mocha:unit',
...
gulp.task('test:server:coverage', gulp.series(cb => {
...
gulp.task('test:e2e', gulp.series(['webpack:e2e', 'env:all', 'env:test', 'start:server', 'webdriver_update'], cb => {
...
gulp.task('build:images', () => {
    return gulp.src(paths.client.images)
        ...
        .pipe(plugins.rev())
        .pipe(gulp.dest(`${paths.dist}/server/${clientPath}/assets/images`))
        .pipe(plugins.rev.manifest(`${paths.dist}/server/${paths.client.revManifest}`, {
            base: `${paths.dist}/server/${clientPath}/assets`,
            merge: true
        }))
        .pipe(gulp.dest(`${paths.dist}/server/${clientPath}/assets`));
});

gulp.task('revReplaceWebpack', function() {
    return gulp.src('dist/server/client/app.*.js')
        .pipe(plugins.revReplace({ manifest: gulp.src(`${paths.dist}/server/${paths.client.revManifest}`) }))
        .pipe(gulp.dest('dist/server/client'));
});
...
gulp.task('copy:extras', () => {
    return gulp.src([
        ...
        .pipe(gulp.dest(`${paths.dist}/server/${clientPath}`));
});
...
gulp.task('copy:fonts:dev', () => {
    return gulp.src('node_modules/{bootstrap,font-awesome}/fonts/*')
        .pipe(flatten())
        .pipe(gulp.dest(`${paths.dist}/server/${clientPath}/assets/fonts`));
});
