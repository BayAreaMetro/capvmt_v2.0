"""Travel Model Run Outputs ETL for VMT Results.

Contributors: Elliot Huang, Kaya Tollas

Required Input Files (in same folder as this script):
- CAPVMT_TAZ_County_Correspondence_TBL.csv
- CAPVMT_TAZ_Places_Correspondence_TBL.csv
- CAPVMT_Place_Lookup.csv
- persons_{model_run}.csv and vmt_{model_run}.csv for each model run defined in MODEL_RUNS

Output File: vmt_results.csv

TODO:
- Replace CAPVMT_Place_Lookup.csv with https://data.bayareametro.gov/dataset/CAPVMT_Place_Lookup/7d3y-eyvu/about_data
- Replace CAPVMT_TAZ_County_Correspondence_TBL.csv with https://data.bayareametro.gov/dataset/CAPVMT_TAZ_County_Correspondence_TBL/aenx-z9d9/about_data
    - First requires taz_list format to be fixed on Socrata (comma-separated string of TAZ IDs)
- Replace CAPVMT_TAZ_Places_Correspondence_TBL.csv with https://data.bayareametro.gov/dataset/CAPVMT_TAZ_Places_Correspondence_TBL/tpnp-wcbv/about_data
    - First requires taz_list format to be fixed on Socrata (comma-separated string of TAZ IDs)
"""
...
from mtcpy.socrata import pull_df_from_socrata

# Note: Update these to the model runs you want to process. The model runs must have the corresponding persons and vmt tables in the same folder as this script.

# These are as of 07/09/2026
MODEL_RUNS = (
    "2023_TM161_IPA_35",
    "2030_TM161_FBP_Plan_01",
    "2035_TM161_FBP_Plan_16",
    "2040_TM161_FBP_Plan_01",
    "2050_TM161_FBP_Plan_16",
)

SOCRATA_LOOKUPS_D = {
    "CAPVMT_TAZ_County_Correspondence_TBL": "aenx-z9d9",  # https://data.bayareametro.gov/Equivalencies/CAPVMT_TAZ_County_Correspondence_TBL/aenx-z9d9/about_data  #noqa
    "CAPVMT_TAZ_Places_Correspondence_TBL": "tpnp-wcbv",  # https://data.bayareametro.gov/Equivalencies/CAPVMT_TAZ_Places_Correspondence_TBL/tpnp-wcbv/about_data  #noqa
    "CAPVMT_Place_Lookup": "7d3y-eyvu",  # https://data.bayareametro.gov/Equivalencies/CAPVMT_Place_Lookup/7d3y-eyvu/about_data  #noqa
}
...
class VMTResultsCalculator:
    ...
    def lookup_taz_list(self, place_id):
        ...
        if place_id < 6000:
            taz_list = tuple(
                [
                    int(taz)
                    for taz in self.taz_city_table.loc[place_id, "Taz_List"].split(",")
                ]
            )
        else:
            taz_list = tuple(
                [
                    int(taz)
                    for taz in self.taz_county_table.loc[place_id, "Taz_List"].split(
                        ","
                    )
                ]
            )
    ...
    def calc_vmt_shares(self, place_id):
        ...
        (
            ...
            v_dest_in,
            v_dest_out,
        ) = self.calc_vmt_table_masks(taz_list)

        # precompute masks for Persons table
        p_live_in, p_live_out, p_work_in, p_work_out, p_work_non = (
            self.calc_persons_table_masks(taz_list)
        )
        ...
        outside = self.vmt_table[
            v_live_in & v_work_in & v_orig_out & v_dest_out
        ].vmt.sum()
        partially_in = self.vmt_table[
            v_live_in
            & v_work_in
            & ((v_orig_in & v_dest_out) | (v_orig_out & v_dest_in))
        ].vmt.sum()
        ...
        inside = self.vmt_table[
            v_live_in & v_work_out & v_orig_in & v_dest_in
        ].vmt.sum()
        outside = self.vmt_table[
            v_live_in & v_work_out & v_orig_out & v_dest_out
        ].vmt.sum()
        partially_in = self.vmt_table[
            v_live_in
            & v_work_out
        ...
        inside = self.vmt_table[
            v_live_in & v_work_non & v_orig_in & v_dest_in
        ].vmt.sum()
        outside = self.vmt_table[
            v_live_in & v_work_non & v_orig_out & v_dest_out
        ].vmt.sum()
        partially_in = self.vmt_table[
            v_live_in
            & v_work_non
        ...
        inside = self.vmt_table[
            v_live_out & v_work_in & v_orig_in & v_dest_in
        ].vmt.sum()
        outside = self.vmt_table[
            v_live_out & v_work_in & v_orig_out & v_dest_out
        ].vmt.sum()
        partially_in = self.vmt_table[
            v_live_out
            & v_work_in
        ...
        inside = self.vmt_table[
            v_live_out & v_work_out & v_orig_in & v_dest_in
        ].vmt.sum()
        outside = self.vmt_table[
            v_live_out & v_work_out & v_orig_out & v_dest_out
        ].vmt.sum()
        partially_in = self.vmt_table[
            v_live_out
            & v_work_out
        ...
        inside = self.vmt_table[
            v_live_out & v_work_non & v_orig_in & v_dest_in
        ].vmt.sum()
        outside = self.vmt_table[
            v_live_out & v_work_non & v_orig_out & v_dest_out
        ].vmt.sum()
        partially_in = self.vmt_table[
            v_live_out
            & v_work_non
...
def calc_vmt_results(model_run):
    ...
    print(
        f"PID:{os.getpid()} {time.ctime()}  Downloading TAZ/county correspondence table"
    )
    # taz_county_table = pd.read_csv(INPUTS_FOLDER/"CAPVMT_TAZ_County_Correspondence_TBL.csv", index_col="County_Domain")

    # Pull from Socrata instead of Box folder
    taz_county_table = (
        pull_df_from_socrata(SOCRATA_LOOKUPS_D["CAPVMT_TAZ_County_Correspondence_TBL"])
        .rename(columns={"county_domain": "County_Domain", "taz_list": "Taz_List"})
        .set_index("County_Domain")
    )

    print(
        f"PID:{os.getpid()} {time.ctime()}  Downloading TAZ/city correspondence table"
    )
    # taz_city_table = pd.read_csv(INPUTS_FOLDER/"CAPVMT_TAZ_Places_Correspondence_TBL.csv", index_col="City_Domain")

    # Pull from Socrata instead of Box folder
    taz_city_table = (
        pull_df_from_socrata(SOCRATA_LOOKUPS_D["CAPVMT_TAZ_Places_Correspondence_TBL"])
        .rename(columns={"city_domain": "City_Domain", "taz_list": "Taz_List"})
        .set_index("City_Domain")
    )

    print(f"PID:{os.getpid()} {time.ctime()}  Downloading Place Name Lookup table")
    # place_lookup_table = pd.read_csv(INPUTS_FOLDER/"CAPVMT_Place_Lookup.csv", index_col="placeid")

    # Pull from Socrata instead of Box folder
    place_lookup_table = (
        pull_df_from_socrata(SOCRATA_LOOKUPS_D["CAPVMT_Place_Lookup"])
        .rename(columns={"cityname": "CityName"})
        .set_index("placeid")
    )

    # download the vmt table
    print(
        f"PID:{os.getpid()} {time.ctime()}  Downloading VMT table for model run '{model_run}'"
    )
    vmt_table = pd.read_csv(INPUTS_FOLDER / f"vmt_{model_run}.csv")

    # download the persons table
    print(
        f"PID:{os.getpid()} {time.ctime()}  Downloading Persons table for model run '{model_run}'"
    )
    persons_table = pd.read_csv(INPUTS_FOLDER / f"persons_{model_run}.csv")

    # instantiate vmt_calculator
    vmt_calculator = VMTResultsCalculator()

    # set the lookup tables
    vmt_calculator.set_taz_correspondence_tables(
        county_table=taz_county_table, city_table=taz_city_table
    )
    ...
    vmt_calculator.set_model_run(
        model_run_id=model_run, vmt_table=vmt_table, persons_table=persons_table
    )
    ...
    for city_id in taz_city_table.index:
        print(
            f"PID:{os.getpid()} {time.ctime()}  Calculating VMT shares for city '{city_id}'"
        )
    ...
    for county_id in taz_county_table.index:
        print(
            f"PID:{os.getpid()} {time.ctime()}  Calculating VMT shares for county '{county_id}'"
        )
    ...
    vmt_results = (
        pd.concat(results)
        .sort_values(by=["CityName", "model_run", "SortOrder2", "SortOrder3"])
...
def calc_all_vmt_results_sequential():
    ...
    taz_county_table = pd.read_csv(
        INPUTS_FOLDER / "CAPVMT_TAZ_County_Correspondence_TBL.csv",
        index_col="County_Domain",
    )

    print(f"{time.ctime()}  Downloading TAZ/city correspondence table")
    taz_city_table = pd.read_csv(
        INPUTS_FOLDER / "CAPVMT_TAZ_Places_Correspondence_TBL.csv",
        index_col="City_Domain",
    )

    print(f"{time.ctime()}  Downloading Place Name Lookup table")
    place_lookup_table = pd.read_csv(
        INPUTS_FOLDER / "CAPVMT_Place_Lookup.csv", index_col="placeid"
    )
    ...
    for m in MODEL_RUNS:
        ...
        vmt_table = pd.read_csv(INPUTS_FOLDER / f"vmt_{m}.csv")
        print(f"{time.ctime()}  Downloading Persons table for model run '{m}'")
        persons_table = pd.read_csv(INPUTS_FOLDER / f"persons_{m}.csv")

        # set the vmt and persons tables in the vmt calculator
        vmt_calculator.set_model_run(
            model_run_id=m, vmt_table=vmt_table, persons_table=persons_table
        )
...
def main():
    ...
    vmt_results_new.to_csv(INPUTS_FOLDER / "vmt_results.csv", index=False)
    print(f"results in folder {INPUTS_FOLDER}")
