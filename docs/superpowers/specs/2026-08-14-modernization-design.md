...
- ~~**ETL-to-Socrata gap risk:** the hand-off from `vmt-results-etl.py`'s output CSV to the live Socrata dataset is undocumented and unscripted.~~ **Mostly closed (2026-08-24):** `etl/publish_to_socrata.py` now scripts this hand-off (`docs/data/etl-to-socrata.md`), verified read-only against the real dataset. Still open: a full write-path rehearsal against a sandbox/staging dataset hasn't happened (needs real model-output CSVs and a non-production dataset, neither available in this environment) — worth doing before the first real publish.
